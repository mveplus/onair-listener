...
def mic_in_use_pipewire(app_hints: str, log: logging.Logger) -> bool:
    """
    PipeWire mic capture detection using pw-dump JSON.

    We look for:
      media.class == Stream/Input/Audio
      and application.name OR application.process.binary matches one of app_hints.

    app_hints can be a comma-separated list, e.g.:
    ...
    try:
        ...
        for o in data:
            p = props(o)
            if (p.get("media.class") or "") != "Stream/Input/Audio":
                continue

            app_name = (p.get("application.name") or "").lower()
            app_bin  = (p.get("application.process.binary") or "").lower()
            hay = f"{app_name} {app_bin}".strip()
        ...
        log.debug("pw-dump mic detection failed: %s", e)
        return False
...
def run_loop(args, state: RuntimeState, lock: threading.Lock, log: logging.Logger):
    ...
    while True:
        ...
        if args.av and meeting_open:
            mic = mic_in_use_pipewire(args.mic_app_hint, log)
            cam = camera_in_use_fuser(video_devices, log)
            with lock:
                state.av = AvSignal(mic=mic, cam=cam, ts=time.time())
            log.info("AV: mic=%s cam=%s (hint=%s)", mic, cam, args.mic_app_hint)
        else:
            with lock:
...
def main():
    ...
    try:
        ...
        print("MEETING_SOURCE=", args.source, "ONAIR_MODE=", args.mode, "AV_DETECTION=", "ON" if args.av else "OFF",
